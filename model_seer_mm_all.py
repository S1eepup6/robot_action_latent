...
s1_pt_name = "/data/libero/exp_results/s1g_seer_mm_all.pt".format(now, SEED)
s2_pt_prefix = "/data/libero/exp_each/s2g_seer_mm_all"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        action_z, pred_agent, pred_wrist = self.seer_module.forward(seer_input_agent, seer_input_wrist, task_embedding_history[:, 0].unsqueeze(-2).float())
        ...
        loss = F.mse_loss(pred_agent, ans) + F.mse_loss(pred_wrist, ans_wrist)
    ...
    def update_stage2(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        ...
        z_policy, next_pred_agent, next_pred_wrist = self.seer_module.forward(first_obs_agent, first_obs_wrist, cur_task)
        ...
        loss = torch.mean(joint_loss) + eef_loss + 0.1 * F.mse_loss(next_pred_agent, ans_agent) + 0.1 * F.mse_loss(next_pred_wrist, ans_wrist)
