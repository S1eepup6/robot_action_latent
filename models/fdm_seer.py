...
class ATTENTION_MODULE_NORM(nn.Module):
    ...
    def _build_mask(self, addition=0):
        ...
        if addition == 0:
            return mask.bool()
        mask_right = torch.zeros((self.total_step, 2 + self.future_step))
        mask_upper = torch.concat([mask, mask_right], dim=1)

        mask_lower = torch.zeros((2 + self.future_step, self.total_step + 2 + self.future_step))
...
class FDM_SEER_BASED(nn.Module):
    def __init__(self,
                 feature_dim = 128,
                 task_embed_dim = 768,
                 total_step = 5,
                 ...
                 output_dim = 128,
                 device = "cpu"):
        super(FDM_SEER_BASED, self).__init__()
        ...
        self.attn_layers.append(ATTENTION_MODULE(feature_dim=feature_dim, total_step=total_step, future_step=future_step, hidden_dim=hidden_dim, output_dim=hidden_dim, use_mask=True, device=device).to(device))
        self.attn_layers.append(nn.Linear(hidden_dim, hidden_dim))
        self.attn_layers.append(nn.GELU())

        for i in range(attn_depth-2): 
            self.attn_layers.append(ATTENTION_MODULE(feature_dim=hidden_dim, total_step=total_step, future_step=future_step, hidden_dim=hidden_dim, output_dim=hidden_dim, use_mask=True, device=device).to(device))
            self.attn_layers.append(nn.Linear(hidden_dim, hidden_dim))
            self.attn_layers.append(nn.GELU())
    ...
    def forward(self, obs, task, action_token):
        B, T, H = obs.shape
...
class ATTENTION_MODULE_CUT(nn.Module):
    ...
    def _build_mask(self, addition=0):
        ...
        if addition == 0:
            return mask.bool()
        mask_right_0 = torch.zeros((self.total_step, 2))
        mask_right_1 = torch.ones((self.total_step, self.future_step))
...
class FDM_SEER_BASED_CUT(nn.Module):
    def __init__(self,
                 feature_dim = 128,
                 task_embed_dim = 768,
                 total_step = 5,
                 ...
                 output_dim = 128,
                 device = "cpu"):
        super(FDM_SEER_BASED_CUT, self).__init__()
        ...
        self.attn_layers.append(ATTENTION_MODULE_CUT(feature_dim=feature_dim, total_step=total_step, future_step=future_step, hidden_dim=hidden_dim, output_dim=hidden_dim, use_mask=True, device=device).to(device))
        self.attn_layers.append(nn.Linear(hidden_dim, hidden_dim))
        self.attn_layers.append(nn.GELU())

        for i in range(attn_depth-2): 
            self.attn_layers.append(ATTENTION_MODULE_CUT(feature_dim=hidden_dim, total_step=total_step, future_step=future_step, hidden_dim=hidden_dim, output_dim=hidden_dim, use_mask=True, device=device).to(device))
            self.attn_layers.append(nn.Linear(hidden_dim, hidden_dim))
            self.attn_layers.append(nn.GELU())
...
if __name__ == "__main__":
    device = 'cuda:1'
    # test_module = FDM_SEER_BASED_CUT(device=device).to(device=device)
    test_module = ATTENTION_MODULE_CUT(device=device).to(device=device)
    ...
    # test_output = test_module(test_input_obs, test_input_task, test_input_act)
