...
class ATTENTION_MODULE_NORM(nn.Module):
    ...
    def _build_mask(self, addition=0):
        ...
        if addition == 0:
            return mask.bool()
        mask_right = torch.zeros((self.image_block, 2))
        mask_upper = torch.concat([mask, mask_right], dim=1)

        mask_lower = torch.zeros((2, self.image_block + 2))
...
class IDM_SEER_NORM(nn.Module):
    ...
    def forward(self, obs, obs_wrist, task):
        ...
        attn_z = torch.concat([obs, obs_wrist, task_z, action_token], dim=-2)
...
class IDM_SEER_STATE(nn.Module):
    def __init__(self,
                 feature_dim = 128,
                 task_embed_dim = 768,
                 total_step = 5,
                 ...
                 output_dim = 128,
                 device = "cpu"):
        super(IDM_SEER_STATE, self).__init__()
        ...
        self.attn_layers.append(ATTENTION_MODULE_NORM(feature_dim=feature_dim, total_step=total_step, multi_modality=multi_modality, hidden_dim=hidden_dim, output_dim=hidden_dim, use_mask=True, device=device).to(device))


        for i in range(attn_depth-2): 
            self.attn_layers.append(ATTENTION_MODULE_NORM(feature_dim=hidden_dim, total_step=total_step, multi_modality=multi_modality, hidden_dim=hidden_dim, output_dim=hidden_dim, use_mask=True, device=device).to(device))
    ...
    def forward(self, obs, obs_wrist, state, task):
        ...
        attn_z = torch.concat([obs, obs_wrist, state, task_z, action_token], dim=-2)
