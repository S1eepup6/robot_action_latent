...
class SEER_ATTN_MODULE(nn.Module):
    ...
    def _build_mask(self, addition=0):
        ...
        if addition == 0:
            return mask.bool()
        mask_right = torch.zeros((self.image_block, 2 + self.future_block))
...
class SEER_ENCODER_STATE(nn.Module):
    ...
    def forward(self, obs, obs_wrist, state, task):
        ...
        attn_z = torch.concat([obs, obs_wrist, state, task_z, pred_token, action_token], dim=-2)
        
        attn_z = self.attn_seq_layer.forward(attn_z)

        action_z = attn_z[:, -1]
        pred_z = attn_z[:, -self.future_step*3-1:-self.future_step*2-1]
        pred_z_wrist = attn_z[:, -self.future_step*2-1:-self.future_step-1]
        pred_z_state = attn_z[:, -self.future_step-1:-1]
...
if __name__ == "__main__":
    device = 'cuda:2'
    test_module = SEER_ENCODER_STATE(device=device).to(device)
