...
s1_pt_name = "/data/libero/exp_results/s1g_seer_mms_all.pt".format(now, SEED)
s2_pt_prefix = "/data/libero/exp_each/s2g_seer_mms_all"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.decoder = DECODER_MM(feature_dim=feature_dim // 2, hidden_dim=hidden_dim, multi_modality=3, output_dim=hidden_dim).to(device)
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        loss = F.mse_loss(pred_agent, ans) + F.mse_loss(pred_wrist, ans_wrist) + F.mse_loss(pred_state, ans_state)
    ...
    def update_stage2(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        losses = []
        ...
        with torch.no_grad():
            ...
            first_obs_agent = obs_embed[:, :time_step]
            ans_agent = obs_embed[:, time_step:]

            first_obs_wrist = obs_embed_wrist[:, :time_step]
            ans_wrist = obs_embed_wrist[:, time_step:]
            
            first_obs_state = obs_embed_state[:, :time_step]
            ans_state = obs_embed_state[:, time_step:]

            cur_task = task_embedding_history[:, 0].unsqueeze(-2)
            

        cur_obs = obs_embed[:, time_step]
        z_policy, next_pred_agent, next_pred_wrist, next_pred_state = self.seer_module.forward(first_obs_agent, first_obs_wrist, first_obs_state, cur_task)
        next_pred_agent = self.predictor_agent(next_pred_agent)
        next_pred_wrist = self.predictor_wrist(next_pred_wrist)
        next_pred_state = self.predictor_state(next_pred_state)

        decoder_input = torch.concat([cur_obs, z_policy], dim=1)
        ...
        loss = torch.mean(joint_loss) + eef_loss + 0.1 * F.mse_loss(next_pred_agent, ans_agent) + 0.1 * F.mse_loss(next_pred_wrist, ans_wrist) + 0.1 * F.mse_loss(next_pred_state, ans_state)
        optim.zero_grad()
        loss.backward()
        optim.step()
    ...
    def update(self, next_data, optim, stage=1, cur_step=0):
        ...
        if stage == 1:
            ...
            return self.update_stage2(obs_history, next_obs, optim, action, cur_step=cur_step)
        else:
...
############################### Code Test ###############################
        ...
        print(m.update(next(replay_dataset), optim, stage=2))
