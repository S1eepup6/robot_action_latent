...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.idm = IDM_MASKED_ATTN(task_embed_dim=task_embed_dim, feature_dim=feature_dim, total_step=self.total_step, hidden_dim=hidden_dim, output_dim=output_dim, device=device).to(device)
        self.a_quantizer = VectorQuantizer(n_code, feature_dim, device=device)
        self.fdm = FDM_MASK_SEL(feature_dim=feature_dim, task_embed_dim=task_embed_dim, total_step=self.time_step, future_step=self.future_step, hidden_dim=hidden_dim, output_dim=output_dim, device=device).to(device)
        ...
        self.decoder = DECODER(feature_dim=feature_dim, hidden_dim=hidden_dim, output_dim=output_dim).to(device)
    ...
    def update_stage3(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        ...
        for cur_obs, next_obs in zip(obs_steps, next_steps):
            ...
            action_loss = self.decoder_head.loss_fn(action, ground_truth_action, reduction='none')
...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        s1_pt_name = "/data/libero/exp_results/s1_tr_fms_{}.pt".format(SEED)
        s2_pt_name = "/data/libero/exp_results/s2_tr_fms_{}.pt".format(SEED)
        s3_pt_name = "/data/libero/exp_results/s3_tr_fms_{}.pt".format(SEED)
        ...
        if STAGE == 3 or STAGE == 5:
            ...
            for task_id in range(26, 45): 
                print(task_id)
                s3_pt_name = "/data/libero/exp_each/s3_tr_fms_task_{}.pt".format(task_id)
                benchmark_dict = benchmark.get_benchmark_dict()
                task_suite = benchmark_dict['libero_90']()
                task = task_suite.get_task(task_id)
                task_name = task.name
