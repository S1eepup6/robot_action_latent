...
def train_one_epoch_libero(
    ...
    for num_steps, batch_calvin in t:
        ...
        # text tokens
        text_tokens = batch_calvin[1].to(device_id, non_blocking=True).unsqueeze(1).repeat(1, window_size, 1)
        ...
        # label action
        label_actions = torch.cat([actions[:, j:sequence_length-atten_goal+j, :].unsqueeze(-2) for j in range(action_pred_steps)], dim=-2) 
        ...
        with autocast():  # image_primary, image_wrist, state, language_instruction
            arm_pred_action, gripper_pred_action, image_pred, arm_pred_state, gripper_pred_state, loss_arm_action = model(
                ...
                action=actions[:, :sequence_length, :],
            )

        # loss_action
        # if loss_action and action_pred_steps:
        ...
        # loss_image 
        label_image_primary = images_primary[:, future_steps:future_steps+sequence_length-atten_goal, :].flatten(0, 1)
        label_image_wrist = images_wrist[:, future_steps:future_steps+sequence_length-atten_goal, :].flatten(0, 1)
        label_image_primary = patchify(label_image_primary, patch_size=patch_size)
        label_image_wrist = patchify(label_image_wrist, patch_size=patch_size)
        ...
        # print(label_image_primary.shape)
        # print(label_image_wrist.shape)

        image_pred = image_pred.reshape(-1, sequence_length, image_pred.shape[1], image_pred.shape[2], image_pred.shape[3])
        # print(image_pred.shape)
        image_pred = image_pred[:, :sequence_length-atten_goal]
        # print(image_pred.shape)
        ...
        if (((num_steps + 1) % gradient_accumulation_steps) == 0) or (
