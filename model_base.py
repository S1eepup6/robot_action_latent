...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.fdm = FDM_MASKED_ATTN(task_embed_dim=task_embed_dim, feature_dim=feature_dim, time_step=self.time_step, hidden_dim=hidden_dim, output_dim=output_dim, device=device).to(device)
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        z = self.idm.forward(obs_embed, task_embedding_history[:, 0].float())
        q_loss, z_q, _, _, _ = self.a_quantizer.forward(z)

        fdm_output = self.fdm.forward(fdm_embed, z_q, task_embedding_history[:, 0].float())
    ...
    def update_stage2(self, obs_history, next_obs, optim, cur_step=0):
        ...
        with torch.no_grad():
            ...
            z = self.idm.forward(obs_embed, task_embedding_history[:, 0])
...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        s1_pt_name = "/data/libero/exp_results/s1_{}_{}.pt".format(now, SEED)
        s2_pt_name = "/data/libero/exp_results/s2_{}_{}.pt".format(now, SEED)
        s3_pt_name = "/data/libero/exp_results/s3_{}_{}.pt".format(now, SEED)
        ...
        if STAGE == 3 or STAGE == 5:
            ...
            for task_id in range(90): 
                print(task_id)
                s3_pt_name = "/data/libero/exp_each/s3_ap_{}_task_{}.pt".format(SEED, task_id)
                benchmark_dict = benchmark.get_benchmark_dict()
                task_suite = benchmark_dict['libero_90']()
                task = task_suite.get_task(task_id)
                task_name = task.name
