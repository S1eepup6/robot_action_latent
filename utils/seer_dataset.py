...
class BaseLiberoDataset(Dataset):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.start_idx = (start_epi) * 50
        self.end_idx = (end_epi+1) * 50
        with open(f"./data_info/{self.dataset_info}.json", 'r') as f:
            self.episode_info_list = json.load(f)
            self.episode_info_list = self.episode_info_list[self.start_idx:self.end_idx:self.partial]
            self.episode_list = [f[0] for f in self.episode_info_list]
            self.num_step_per_episode = [f[1] - self.max_window_size for f in self.episode_info_list]
            self.num_episode = len(self.episode_list)
...
def get_libero90_finetune_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    dataset_names = ["libero_90_converted"]
    shared_epoch = SharedEpoch(epoch=epoch)
    preprocess_image_fn = functools.partial(
        preprocess_image, image_processor=image_processor
...
def get_liberogoal_dataset(seed, batch_size, window_size, image_processor, tokenizer, epoch=0, start_epi=0, end_epi=9, floor=False, partial=1):
    ...
    libero_dataset = DiskLiberoDataset(
        ...
        text_aug=False,
        act_step=1,
        root_dir="/home/dls0901/data_seer/",
        image_primary_size=200,
        image_wrist_size=84,
        ...
        small_size=0,
        dataset_info='libero_goal_converted',
        gripper_width=True,
        load_libero_file="h5",
        start_epi=start_epi,
        end_epi=end_epi,
