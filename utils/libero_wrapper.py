...
def make(task_id, task_suite_name, seed=1, train=False, img_size=128, device_id=-1, libero_path="/data/libero"):
    benchmark_dict = benchmark.get_benchmark_dict()
    task_suite = benchmark_dict[task_suite_name]()
    task = task_suite.get_task(task_id)
    task_name = task.name
    task_description = task.language
    task_bddl_file = os.path.join(f"{libero_path}/LIBERO/libero/libero/bddl_files", task.problem_folder, task.bddl_file)
