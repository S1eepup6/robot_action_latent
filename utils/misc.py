...
def compute_traj_latent_embedding(episode, device, nstep_history):
    with torch.no_grad():
        ...
        for t in range(obs_agent.shape[0]):
            ...
            obs_agent_history = torch.concat(list(obs_agent_buffer), dim=0) ### (10,3,128,128)
            obs_wrist_history = torch.concat(list(obs_wrist_buffer), dim=0) ### (10,3,128,128)
            state_history = torch.concat(list(state_buffer), dim=0) ### (10,state_dim)
            task_embedding_history = torch.concat(list(task_embedding_buffer), dim=0) ###(10,feature_dim)(10,lang_emb_dim)
        ...
        obs_agent_episode = torch.concat(obs_agent_episode, dim=0)
        obs_wrist_episode = torch.concat(obs_wrist_episode, dim=0)
        state_episode = torch.concat(state_episode, dim=0)
        task_embedding_episode = torch.concat(task_embedding_episode, dim=0)
