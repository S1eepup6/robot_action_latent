...
from models.seer_encoder_mm_2 import *
...
s1_pt_name = "/data/libero/exp_results/s1g_seer_mms_ori2_ones.pt".format(now, SEED)
s2_pt_prefix = "/data/libero/exp_each/s2g_seer_mms_ori2_ones"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    ...
    def update_stage1(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        ...
        obs_agent_history = obs_agent_history.reshape(-1, num_channel, img_size, img_size).float()
        obs_embed = self.encoder.forward(obs_agent_history)
        seer_input_agent = obs_embed.reshape(batch_size, time_step, -1)

        obs_wrist_history = obs_wrist_history.reshape(-1, num_channel, img_size, img_size).float()
        obs_embed_wrist = self.encoder_wrist.forward(obs_wrist_history)
        seer_input_wrist = obs_embed_wrist.reshape(batch_size, time_step, -1)
        
        state_history = state_history.float()
        seer_input_state = self.encoder_state.forward(state_history)
        ...
        with torch.no_grad():
            next_obs_agent = next_obs_agent.reshape(-1, num_channel, img_size, img_size).float()
            next_obs_agent = self.encoder.forward(next_obs_agent)
            ans = next_obs_agent.reshape(batch_size, next_time_step, -1)

            next_obs_wrist = next_obs_wrist.reshape(-1, num_channel, img_size, img_size).float()
            next_obs_wrist = self.encoder_wrist.forward(next_obs_wrist)
            ans_wrist = next_obs_wrist.reshape(batch_size, next_time_step, -1)
            
            next_state = next_state.float()
            ans_state = self.encoder_state.forward(next_state)
