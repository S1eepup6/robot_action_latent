...
for i in tqdm(range(45)):
    ...
    agent_path = "/data/libero/exp_each/s2_seer_task_{}.pt".format(i)
    print(agent_path)
    ...
    while eval_until_episode(episode):
        ...
        while step < EVAL_MAX_STEP:
            ...
            with torch.no_grad():
                ...
                while len(obs_history) < 8:
                    obs_history = obs_history + [cur_obs_embed.unsqueeze(-2), ]


                history_obs_embed = torch.concat(obs_history, dim=1)
                if step % 4 == 0:
                    z_q, _ = agent.seer_module(history_obs_embed, task_embedding)   


                decoder_input = torch.concat([cur_obs_embed, z_q], dim=1)
                ...
                eef_action = F.softmax(eef_action, dim=-1)
                eef_action = torch.argmax(eef_action, dim=-1).detach().cpu().numpy()
    ...
    try:
        with open("performance_seer.pkl", 'rb') as f:
            performance = pickle.load(f)
    except:
        performance = {}
    with open("performance_seer.pkl", 'wb') as f:
