...
from model_taskrevise_enc import *
...
for i in tqdm(range(45)):
    ...
    agent_path = "/data/libero/exp_each/s3_task_revise_enc_{}.pt".format(i)
    print(agent_path)
    ...
    while eval_until_episode(episode):
        ...
        while step < EVAL_MAX_STEP:
            ...
            with torch.no_grad():
                ...
                task_z = agent.task_encoder(task_embedding)

                if step % 4 == 0 or q_res is None:
                    hl_policy_input = torch.concat([cur_obs, task_z], dim=1).float()
                ...
                action = agent.decoder(z_q)
                # print(action.shape)
                action = action[:, 512*(action_step_i):512*(action_step_i+1)]
                # print(action.shape)
    ...
    try:
        with open("performance_task_revise_enc.pkl", 'rb') as f:
            performance = pickle.load(f)
    except:
        performance = {}
    with open("performance_task_revise_enc.pkl", 'wb') as f:
