...
from model_hotl_seer_mm import *
...
RESULT_FILE_NAME = "performance_hotl_seer_mm.pkl"
AGENT_PATH_PREFIX = "/data/libero/exp_each/s3_hotl_seer_mm"
#################### ARGUMENTS #####################
        ...
        while step < EVAL_MAX_STEP:
            ...
            with torch.no_grad():
                ...
                if step % N_FUTURE == 0 or q_res is None:
                    hl_policy_input = torch.concat([cur_obs, cur_obs_wrist, task_embedding], dim=1).float()

                    z_policy = agent.hl_policy(hl_policy_input)   
                    q_loss, z_q, _, _, _ = agent.a_quantizer.forward(z_policy)


                decoder_input = torch.concat([cur_obs, cur_obs_wrist, z_q], dim=1)
