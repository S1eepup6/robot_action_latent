...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.idm = IDM_MASKED_ATTN(task_embed_dim=feature_dim, feature_dim=feature_dim, total_step=self.total_step, hidden_dim=hidden_dim, output_dim=output_dim, device=device).to(device)
        self.a_quantizer = VectorQuantizer(n_code, feature_dim, device=device)
        self.fdm = FDM_MASKED_ATTN(task_embed_dim=feature_dim, feature_dim=feature_dim, time_step=self.time_step, hidden_dim=hidden_dim, output_dim=output_dim, device=device).to(device)

        self.hl_policy = HL_PRED(feature_dim=feature_dim, task_embed_dim=feature_dim, hidden_dim=[hidden_dim, hidden_dim, hidden_dim, hidden_dim], output_dim=output_dim).to(device)

        self.decoder = DECODER_L(feature_dim=feature_dim, hidden_dim=hidden_dim, output_dim=output_dim).to(device)
        self.decoder_head = GMMHead(output_dim, action_dim-1, hidden_dim, loss_coef=0.01)
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        history_step = obs_agent_history.shape[1]
        ...
        task_z = self.task_encoder(task_embedding_history[:, 0].float())
        z = self.idm.forward(obs_embed, task_z)
        q_loss, z_q, _, _, _ = self.a_quantizer.forward(z)

        fdm_output = self.fdm.forward(fdm_embed, z_q, task_z)
        fdm_output = fdm_output.reshape(batch_size, time_step, -1)

        ans = obs_embed[:, history_step:]
    ...
    def update_stage2(self, obs_history, next_obs, optim, cur_step=0):
        ...
        with torch.no_grad():
            ...
            task_z = self.task_encoder(task_embedding_history[:, 0])
            z = self.idm.forward(obs_embed, task_z)


        cur_obs = obs_embed[:, 0].flatten(start_dim=1)
        target_obs = obs_embed[:, -1].flatten(start_dim=1)
        cur_task = self.task_encoder(task_embedding_history[:, 0])
        ...
        if self.writer is not None:
            self.writer.add_scalar("loss/stage2_policy", loss_policy.item(), cur_step)
            self.writer.add_scalar("loss/stage2_pred", loss_pred.item(), cur_step)
    ...
    if TRAIN:
        ...
        # Dataset Load
            ...
            replay_loader = make_replay_loader_dist(
                            ...
                            save_snapshot=True, nstep=4, nstep_history=4, 
        ...
        s1_pt_name = "/data/libero/exp_results/s1_trl_{}.pt".format(SEED)
        s2_pt_name = "/data/libero/exp_results/s2_trl_{}.pt".format(SEED)
        s3_pt_name = "/data/libero/exp_results/s3_trl_{}.pt".format(SEED)

        # s2_pt_name = "/data/libero/exp_results/s2_06-19_23:29_0.pt".format(now, SEED)
        ...
        if STAGE == 1 or STAGE == 4 or STAGE == 5:
            ...
            for i in tqdm(range(100010)):
                m.update(next(replay_dataset), optim, stage=1, cur_step=i)
        ...
        if STAGE == 2 or STAGE == 4 or STAGE == 5:
            ...
            # Stage 2
            for i in tqdm(range(100010)):
                m.update(next(replay_dataset), optim, stage=2, cur_step=i)
        ...
        if STAGE == 3 or STAGE == 5:
            ...
            for task_id in range(3, 45): 
                print(task_id)
                s3_pt_name = "/data/libero/exp_each/s3_trl_{}.pt".format(task_id)
                benchmark_dict = benchmark.get_benchmark_dict()
                task_suite = benchmark_dict['libero_90']()
                task = task_suite.get_task(task_id)
                task_name = task.name
