...
s1_pt_name = "/data/libero/exp_results/s1_hotl_seer_mms_90g.pt".format(now, SEED)
s2_pt_name = "/data/libero/exp_results/s2_hotl_seer_mms_90g.pt".format(now, SEED)
s3_pt_prefix = "/data/libero/exp_each/s3_hotl_seer_mms_90g"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        loss = F.mse_loss(pred_agent, ans) + F.mse_loss(pred_wrist, ans_wrist) + F.mse_loss(pred_state, ans_state) + q_loss
    ...
    def update_stage2(self, obs_history, next_obs, optim, cur_step=0):
        ...
        with torch.no_grad():
            ...
            action_z, _, _, _ = self.seer_module.forward(obs_embed[:, :time_step], obs_embed_wrist[:, :time_step], obs_embed_state[:, :time_step], task_embedding_history[:, 0].unsqueeze(1))


        cur_obs = obs_embed[:, time_step].flatten(start_dim=1)
        cur_obs_wrist = obs_embed_wrist[:, time_step].flatten(start_dim=1)
        cur_obs_state = obs_embed_state[:, time_step].flatten(start_dim=1)

        target_obs = torch.concat([obs_embed[:, -1].flatten(start_dim=1), obs_embed_wrist[:, -1].flatten(start_dim=1), obs_embed_state[:, -1]], dim=1)

        cur_task = task_embedding_history[:, 0]
        hl_policy_input = torch.concat([cur_obs, cur_obs_wrist, cur_obs_state, cur_task], dim=1).float()

        z_policy, pred_obs = self.hl_policy.forward_with_pred(hl_policy_input)   

        loss_policy = F.mse_loss(z_policy, action_z)
        loss_pred = F.mse_loss(pred_obs, target_obs)
    ...
    def update_stage3(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        ...
        with torch.no_grad():
            ...
            first_obs_state = obs_embed_state[:, 0].flatten(start_dim=1)
            cur_task = task_embedding_history[:, 0]
            
            for i in range(self.time_step):
                obs_steps.append([obs_embed[:, i].flatten(start_dim=1), obs_embed_wrist[:, i].flatten(start_dim=1), obs_embed_state[:, i]])
                next_steps.append([obs_embed[:, i+1].flatten(start_dim=1), obs_embed_wrist[:, i+1].flatten(start_dim=1), obs_embed_state[:, i+1]])
