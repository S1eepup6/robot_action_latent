...
from models.seer_encoder import *
...
s1_pt_name = "/data/libero/exp_results/s1_seer.pt".format(now, SEED)
s2_pt_prefix = "/data/libero/exp_each/s2_seer"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        loss = F.mse_loss(pred_output, ans)
    ...
    def update_stage2(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        ...
        with torch.no_grad():
            ...
            first_obs = obs_embed[:, :time_step]
            next_ans = obs_embed[:, time_step:]
            ...
            hl_policy_input = [first_obs, cur_task]

        cur_obs = obs_embed[:, time_step]
        z_policy, next_pred = self.seer_module.forward(*hl_policy_input)
        next_pred_obs = self.predictor(next_pred)
        ...
        loss = torch.mean(joint_loss) + eef_loss + 0.1 * F.mse_loss(next_pred_obs, next_ans)
        ...
        if STAGE == 2 or STAGE == 5:
            ...
            for task_id in range(45): 
                ...
                try:
                    ...
                    print("VALUE ERROR at Task ".format(task_id))
