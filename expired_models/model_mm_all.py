...
from models.idm_mm import IDM_SEER_NORM
from models.fdm_mm import FDM_SEER_NORM
...
s1_pt_name = "/data/libero/exp_results/s1g_mm_all.pt".format(now, SEED)
s2_pt_name = "/data/libero/exp_results/s2g_mm_all.pt".format(now, SEED)
s3_pt_prefix = "/data/libero/exp_each/s3g_mm_all"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.idm = IDM_SEER_NORM(feature_dim=feature_dim, 
        ...
        self.fdm = FDM_SEER_NORM(feature_dim=feature_dim, 
                                  task_embed_dim=task_embed_dim, 
        ...
        self.hl_policy = HL_PRED_MM(feature_dim=feature_dim, task_embed_dim=task_embed_dim, hidden_dim=[hidden_dim*2, hidden_dim*2, hidden_dim, hidden_dim], output_dim=output_dim).to(device)

        self.decoder = DECODER_L_MM(feature_dim=feature_dim, hidden_dim=hidden_dim, output_dim=output_dim).to(device)
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        z = self.idm.forward(obs_embed, obs_embed_wrist, task_embedding_history[:, 0].float().unsqueeze(-2))
        q_loss, z_q, _, _, _ = self.a_quantizer.forward(z)

        fdm_agent, fdm_wrist = self.fdm.forward(fdm_embed, fdm_embed_wrist, task_embedding_history[:, 0].float().unsqueeze(-2), z_q.unsqueeze(-2))
        fdm_agent = fdm_agent.reshape(batch_size, next_time_step, -1)
        fdm_wrist = fdm_wrist.reshape(batch_size, next_time_step, -1)

        loss = F.mse_loss(fdm_agent, ans) + F.mse_loss(fdm_wrist, ans_wrist) + q_loss
    ...
    def update_stage2(self, obs_history, next_obs, optim, cur_step=0):
        ...
        with torch.no_grad():
            ...
            z = self.idm.forward(obs_embed, obs_embed_wrist, task_embedding_history[:, 0].unsqueeze(-2))
        ...
        ans = torch.concat([target_obs, target_obs_wrist], dim=-1)

        cur_task = task_embedding_history[:, 0]
        hl_policy_input = torch.concat([cur_obs, cur_obs_wrist, cur_task], dim=1).float()
    ...
    def update_stage3(self, obs_history, next_obs, optim, ground_truth_action, cur_step=0):
        ...
        with torch.no_grad():
            ...
            for i in range(self.time_step):
                obs_steps.append([obs_embed[:, i].flatten(start_dim=1), obs_embed_wrist[:, i].flatten(start_dim=1)])
                next_steps.append([obs_embed[:, i+1].flatten(start_dim=1), obs_embed_wrist[:, i+1].flatten(start_dim=1)])

            hl_policy_input = torch.concat([first_obs, first_obs_wrist, cur_task], dim=1).float()
...
if __name__ == "__main__":
    ...
    DATA_STORAGE_DIR = "/home/dls0901/data_libero_prise"
