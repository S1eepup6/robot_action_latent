...
from models.idm_seer import IDM_SEER_BASED
from models.fdm_seer import FDM_SEER_BASED
...
s1_pt_name = "/data/libero/exp_results/s1_hotl_sd.pt".format(now, SEED)
s2_pt_name = "/data/libero/exp_results/s2_hotl_sd.pt".format(now, SEED)
s3_pt_prefix = "/data/libero/exp_each/s3_hotl_sd"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.idm = IDM_SEER_BASED(feature_dim=feature_dim, 
        ...
        self.fdm = FDM_SEER_BASED(feature_dim=feature_dim, 
                                  task_embed_dim=task_embed_dim, 
    ...
    def update_stage1(self, obs_history, next_obs, optim, cur_step=0):
        ...
        z = self.idm.forward(obs_embed, task_embedding_history[:, 0].float().unsqueeze(-2))
        q_loss, z_q, _, _, _ = self.a_quantizer.forward(z)

        fdm_output = self.fdm.forward(fdm_embed, task_embedding_history[:, 0].float().unsqueeze(-2), z_q.unsqueeze(-2))
        fdm_output = fdm_output.reshape(batch_size, next_time_step, -1)
    ...
    def update_stage2(self, obs_history, next_obs, optim, cur_step=0):
        ...
        with torch.no_grad():
            ...
            z = self.idm.forward(obs_embed, task_embedding_history[:, 0].unsqueeze(-2))
