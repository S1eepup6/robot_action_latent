...
s1_pt_name = "/data/libero/exp_results/s1_hotl_base.pt".format(now, SEED)
s2_pt_name = "/data/libero/exp_results/s2_hotl_base.pt".format(now, SEED)
s3_pt_prefix = "/data/libero/exp_each/s3_hotl_base"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    def __init__(self,
        ...
        self.fdm = FDM_ALL_MASK(feature_dim=feature_dim, task_embed_dim=task_embed_dim, total_step=self.time_step, future_step=self.time_step, hidden_dim=hidden_dim, output_dim=output_dim, device=device).to(device)
