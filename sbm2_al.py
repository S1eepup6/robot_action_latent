...
s1_pt_name = "/data/libero/exp_results/sbm2_al.pt".format(now, SEED)
s2_pt_prefix = "/data/libero/exp_each/sbm2_al2"
#################### ARGUMENTS #####################
...
class MYMODEL(nn.Module):
    ...
    def update_stage1(self, libero_dataset, num_epochs, save_pt_name=None):
        ...
        for i in range(num_epochs):
            ...
            if save_pt_name is not None:
                torch.save(self.state_dict(), save_pt_name)

                
    def update_stage2(self, libero_dataset, num_epochs, save_pt_name=None):
        num_batches = libero_dataset.dataloader.num_batches * 1
        optim = torch.optim.AdamW(self.seer_model.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    def update(self, libero_loader, num_epochs, save_pt_name=None, stage=1):
        if stage == 1:
            return self.update_stage1(libero_loader, num_epochs, save_pt_name=save_pt_name)
        elif stage == 2:
            return self.update_stage2(libero_loader, num_epochs, save_pt_name=save_pt_name)
        else:
...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        m.update(libero_dataset, N_EPOCH, save_pt_name=s1_pt_name, stage=1)
        ...
        s2_pt_name = s2_pt_prefix + ".pt"
        m.update(libero_dataset, N_EPOCH_2, save_pt_name=s2_pt_name, stage=2)
