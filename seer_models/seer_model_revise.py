...
def generate_attention_mask(K, num_A, num_B, atten_goal, atten_goal_state,
    ...
    for i in range(K):
        ...
        if num_obs_token > 0 and atten_only_obs and action_pred_steps:
            attention_mask[start_index+num_A+action_pred_steps:start_index+num_A+action_pred_steps+num_obs_token] = -float('inf')
            attention_mask[start_index+num_A+action_pred_steps:start_index+num_A+action_pred_steps+num_obs_token, start_index+2:start_index+num_A] = 0.0
            attention_mask[start_index+num_A+action_pred_steps:start_index+num_A+action_pred_steps+num_obs_token, start_index+num_A:start_index+num_A+action_pred_steps] = 0.0 
            if attn_robot_proprio_state:
                attention_mask[start_index+num_A+action_pred_steps:start_index+num_A+action_pred_steps+num_obs_token, start_index+1:start_index+2] = 0.0
