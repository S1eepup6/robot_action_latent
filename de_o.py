    ...
    def update(self, data_loader, epoch, make_log = False, save_file_name = None):
        ...
        for e in tqdm(range(epoch)):
            ...
            for data in epoch_pbar:
                ...
                obs = obs.to(DEVICE)
                gt_act = gt_act.to(DEVICE).unsqueeze(0)
                final_goal = final_goal.to(DEVICE)

                with torch.no_grad():
                    obs_enc = self.encoder(obs).flatten(start_dim=-2).unsqueeze(0)
                    goal_enc = self.encoder(final_goal).flatten(start_dim=-2).repeat(WINDOW_SIZE, 1).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        m.update(train_loader, FINETUNE_EPOCH, make_log=True, save_file_name=s1_pt_name)
